...
| `openblas_core_type` | `OPENBLAS_CORETYPE` | `HASWELL`; numerical kernel profile. |
...
with `--params=<file>`, remain readable. The launcher reports a deprecation
warning but does not rewrite those files. A legacy project can be migrated by
translating its Make parameters to `scbolt.yml`, retaining `spec.yml` through
the `spec_file` key, and changing the locator:
