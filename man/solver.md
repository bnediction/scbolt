...
`bn-min` enumerates globally minimum-interaction networks without optimizing
the number of constants. When `minimize-self-loops-inference` is enabled, its
lexicographic objective first minimizes the total number of interactions and
then minimizes one-node feedbacks as a tie-breaker. `inference-limit` bounds the
number of tied optima generated; an empty value or zero enumerates all of them.

| Parameter | Meaning |
| --- | --- |
| `minimize-self-loops-inference` | Whether `bn-min` additionally minimizes one-node feedbacks. |
| `inference-limit` | Maximum number of networks generated by `bn-min`, `bn-submin`, or `bn-diverse`; an empty value or zero enumerates all available solutions. |
| `configuration-formats` | Configuration export formats used by final Boolean-network inference. |
