...
mostly due to genome-scale network complexity. The semi-automatic workflow scbolt
...
in reconstructing data-driven Boolean networks. Also, scbolt aims to provide an answer to the following challenges:
...
space. To achieve these goals, scbolt proposes a guideline made up of many computational
steps (see Figure below). The first ones are classic methods for the
feature counting and preprocessing. As scbolt aims to decipher biolgical processes from
...
can be run to analyze the temporal cell behaviors. Then scbolt searches for key phenotypic manifolds,
...
Using as domain a gene-regulatory network such as Collectri, scbolt can then infer BNs
...
For a proper use of the pipeline, an advanced documentation is provided [here](https://github.com/bnediction/scbolt/tree/main/man).
