...
parser = argparse.ArgumentParser(
    ...
    usage="python potency.py <FILE> <FILE> [<args>]",
...
std.print_debug(f"smoothing batch: [chunk: {smooth_chunk_number}, cpu cores: {cores_to_use_for_smooth_batch}]")

model = torch.load(Path(f"{os.path.dirname(inspect.getfile(cytotrace))}/resources/background.pt"))
model = model.t()
...
potency_df.columns = potency_df.columns.map(lambda x: re.sub(r'cytotrace2_', "", x, flags=re.IGNORECASE).lower())
...
if args.csv:
    ...
    potency_df.to_csv(
        os.path.join(args.outpath, args.csv),
        sep=",",
        index=True
    )
...
twin_ax = ax.twinx()
twin_ax.set_yticks(np.linspace((ylim_max-ylim_min)/(2*len(labels)),1-((ylim_max-ylim_min)/(2*len(labels))),len(labels)))
...
if args.h5ad:
    std.print_task(f"saving predicted cell potencies in {os.path.join(args.outpath, args.h5ad)}")
    adata.write_h5ad(
        filename=os.path.join(args.outpath, args.h5ad),
        compression="gzip"
    )
