...
parser.add_argument(
    ...
    type=lambda x: Path(x).resolve(),
    metavar="FILE",
    help="output file containing gene ontology enrichment results (xlsx format)"
)
...
std.print_task(f"loading gene-to-go associations from {args.gene2go if args.gene2go else args.annotations}")
...
with ExcelWriter(args.outfile) as xlsx_writer:
    for cluster in study_geneset.keys():
        ...
        if os.path.isfile(xlsx_infile):
            ...
            goea_results.sort_values(
                by="p_fdr_bh",
                axis=0,
                ascending=True
            )
            goea_results.to_excel(
                xlsx_writer,
                sheet_name=cluster
            )
