...
""",
    ...
    with open(args.model_specification, "r") as file:
        raw_specification = yaml.safe_load(file) or {}

    try:
        specification = normalize_model_specification(raw_specification)
    except ValueError as error:
        parser.error(str(error))

    dynamical_constraints = specification["constraints"]
    important_nodes = set(specification["important-nodes"])
    mandatory_nodes = set(specification["mandatory-nodes"])
